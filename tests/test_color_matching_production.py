...
class ColorMatchingProductionTest(unittest.IsolatedAsyncioTestCase):
    def test_brand_color_matrix_uses_production_matcher(self) -> None:
        accepted = [
            (
                ...
                "Google Pixel 8 8GB/128GB мятный зелёный",
            ),
            (
        ...
        for canonical, candidate, expected_reason in rejected:
    ...
    def test_no_selected_color_does_not_filter_model_words(self) -> None:
        self.assertIsNone(
            CatalogFirstPriceService._model_mismatch_reason(
